...
NSString * HawaiiFarmersMarketGuide$ModuleRequireFormat = @"(function(exports){"
...
@implementation HawaiiFarmersMarketGuideObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_hawaiifarmersmarketguide);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create HawaiiFarmersMarketGuide global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "HawaiiFarmersMarketGuide" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_hawaiifarmersmarketguide = [[HawaiiFarmersMarketGuideObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_hawaiifarmersmarketguide];
	
	NSString *_hawaiifarmersmarketguideNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _hawaiifarmersmarketguideNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_hawaiifarmersmarketguide valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:HawaiiFarmersMarketGuide$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.hawaiifarmersmarketguide.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.hawaiifarmersmarketguide.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
