...
//Rare exceptions to indicate a bug in the _hawaiifarmersmarketguide code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_hawaiifarmersmarketguide = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_hawaiifarmersmarketguide]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.hawaiifarmersmarketguide.module"
