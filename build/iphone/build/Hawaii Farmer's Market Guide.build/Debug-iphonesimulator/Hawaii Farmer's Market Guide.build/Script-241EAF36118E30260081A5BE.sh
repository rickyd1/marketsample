...
if [ "x$TITANIUM_CLI_XCODEBUILD" == "x" ]; then
    /usr/local/Cellar/node/0.10.28/bin/node "/usr/local/bin/titanium" build --platform iphone --sdk "3.4.0.GA" --no-prompt --no-progress-bars --no-banner --no-colors --build-only --xcode
    exit $?
else
